...
function buildPokeCard(p) {{
  ...
  // Habitats (suppressed for NPCs and dex-only entries)
  // Look up actual habitat names rather than showing stored type keywords,
  // so terrain and decoration habitats display consistently.
  if (!p.npc && !p.dex_only) {{
    const myHabs = HABITATS.filter(h => (h.pokemon || []).includes(p.name));
    if (myHabs.length > 0) {{
      card.appendChild(makeDivider());
      const lbl = document.createElement('div');
      lbl.className = 'section-lbl';
      lbl.textContent = myHabs.length > 1 ? 'Habitats' : 'Habitat';
      card.appendChild(lbl);
      const row = document.createElement('div');
      row.className = 'info-row';
      myHabs.forEach(h => {{
        ...
        chip.title = 'View all habitats for this Pokémon';
